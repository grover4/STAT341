...
- Any calculations - other than simple ones - should be done in a code chunk with the answer printed. You can practice saving the answer to a variable, and printing the value of the variable. You can also additionally report it using inline code.
...
$$S = X_1 + X_2 + \dots + X_{k} \sim Pois(\sum\limits_{i=1}^{k} \lambda_i).$$
    ...
    *Hint*: For inspiration, look at Example 3.1 and the proof of Theorem 3.2. Clearly state the proposition you are trying to prove, the proof for the base case, and the inductive step
...
c. By how much will the daily cost vary from expected? Calculate the standard deviation of the random variable $C$.  Be sure to show and support your work. Comment on assumptions you need to make for and also \textbf{whether or not they are justified}
