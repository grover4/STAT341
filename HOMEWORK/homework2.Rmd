...
1. (Fill in the blanks) The random variables $X$ and $Y$ are independent, each taking the values 1, 2 or 3. Complete the following table of the joint PMF. Show your work for each entry below the table.
...
2. (Two friends) Two friends - let's call them Krish and Kian - agree to meet at Tully's for coffee. Suppose that the random variables 
$$X =  \mbox{the time that Krish arrives at Tully's and}$$
$$Y = \mbox{the time that Kian arrives at Tully's}$$
...
c. Calculate the probability that Krish arrives before Kian. Show your steps. (Do the integration, do not just use geometry)
...
a. Let the random variable $X_k$ denote her winnings on the $kth$ hand. Write the PMF of $X_k$ by filling in the empty cells in the table below.
...
b. Find $\mu_k = E\left[X_k\right]$. 

c. Let the random variable $T$ denote the total amount she wins in $n$ hands, that is $T=X_1+X_2+\dots+X_n$.  Find $E\left[T\right]$. Do we need to assume that the $X_k$ to be independent for finding $E\left[T \right]$? 

d. Find $Var\left[T\right]$. State assumptions (in context) you need to make about the $X_k$ in order to do your calculation.
