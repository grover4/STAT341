...
```{r eval=F,echo=F}
library(tidyverse)
#read data into R object called airbnb. 
#
airbnb <- read_csv(file="airbnb.csv")

airbnb_subset <- airbnb %>%
               filter(property_type=="Apartment", beds==2) %>%
                select(price) %>% 
                mutate(price = parse_number(price)) 

write_csv(airbnb_subset, file="airbnb_price.csv")
```
